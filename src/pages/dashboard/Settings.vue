...
// Pending Requests for Admin Approval
const pendingApprovals = ref([
  {
    ...
    title: "Privilege Change Request",
    description:
      "User is requesting Editor privilege for Main Production system",
    userName: "sarah_smith",
    userEmail: "sarah@nanopass.com",
    requestedPrivilege: "Editor",
    system: "Main Production",
    currentPrivilege: "Viewer",
    reason: "Need to modify content for new campaign",
    createdAt: "2024-12-08 10:30:00",
    priority: "normal",
  },
  {
    id: 2,
    type: "password_access",
    title: "App Password Access Request",
    description: "User is requesting access to Development system password",
    userName: "mike_johnson",
    userEmail: "mike@nanopass.com",
    system: "Development",
    purpose: "Automated backup script integration",
    reason: "Backup automation setup",
    createdAt: "2024-12-07 15:45:00",
    priority: "high",
  },
  {
    id: 3,
    type: "privilege_change",
    title: "Privilege Change Request",
    description: "User is requesting Admin privilege for Staging system",
    userName: "alex_davis",
    userEmail: "alex@nanopass.com",
    requestedPrivilege: "Admin",
    system: "Staging Environment",
    currentPrivilege: "Editor",
    reason: "Need full access for testing and debugging",
    createdAt: "2024-12-06 09:20:00",
    priority: "high",
  },
]);
...
const confirmRequestData = ref(null);
...
const openConfirmModal = (action, id) => {
  const request = pendingApprovals.value.find((r) => r.id === id);
  if (request) {
    confirmAction.value = action;
    confirmRequestId.value = id;
    confirmRequestData.value = request;
...
const confirmApproveOrReject = () => {
  if (confirmAction.value === "approve") {
    // Add to user's approved requests
    requests.value.unshift({
      ...confirmRequestData.value,
      status: "approved",
    });
  } else if (confirmAction.value === "reject") {
    // Add to user's rejected requests
    requests.value.unshift({
      ...confirmRequestData.value,
      status: "rejected",
    });
  ...
  pendingApprovals.value = pendingApprovals.value.filter(
    (r) => r.id !== confirmRequestId.value
  );
  // Close modal and reset
  showConfirmModal.value = false;
  confirmAction.value = null;
  confirmRequestId.value = null;
  confirmRequestData.value = null;
};
    ...
    <div class="p-6">
      ...
      <div class="mb-8">
        ...
        <p class="mt-2 text-gray-600">
          Manage your profile, access permissions, and security settings
        </p>
      </div>
      ...
      <div class="mb-8 border-b border-gray-200">
        <div class="flex gap-8">
          ...
          >
            <ThumbsUp class="inline mr-2 h-4 w-4" />
            Approve
          </button>
        </div>
      </div>
        ...
        <div class="rounded-lg border bg-white p-6 shadow-sm">
          ...
          <div class="grid gap-6 md:grid-cols-2">
            ...
            <div>
              ...
              >
              <div
                class="px-4 py-2 bg-gray-50 rounded-md border border-gray-200 text-gray-900 text-sm"
              >
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              <p class="text-sm text-blue-800 mt-1">
                ...
                administrator or visit the security settings page.
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="rounded-lg border bg-white shadow-sm overflow-hidden">
          <div class="border-b bg-gray-50 px-6 py-4">
            ...
            <p class="text-sm text-gray-600 mt-1">
              Manage your active sessions and revoke access if needed
            </p>
          </div>
      ...
      <div v-show="activeTab === 'passwords'" class="space-y-6">
        <div class="flex items-center justify-between mb-6">
          <div>
            ...
            <p class="text-sm text-gray-600 mt-1">
              Create and manage passwords for automated access and integrations
            </p>
          </div>
          <button
            @click="showNewAppPasswordModal = true"
            class="inline-flex items-center gap-2 px-4 py-2 bg-black text-white text-sm font-medium rounded-md hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-black"
          >
            <Plus class="h-4 w-4" />
            New App Password
        ...
        <div class="rounded-lg border bg-white shadow-sm overflow-hidden">
          <table class="w-full">
            ...
            <tbody class="divide-y">
              <tr
                v-for="password in appPasswords"
                :key="password.id"
              ...
              >
                ...
                <td class="px-6 py-4 text-right">
                  <button
                    @click="revokeAppPassword(password.id)"
      ...
      <div v-show="activeTab === 'requests'" class="space-y-6">
        <div class="flex items-center justify-between mb-6">
          <div>
            ...
            <p class="text-sm text-gray-600 mt-1">
              Submit and track privilege and access requests
            </p>
          </div>
          <button
      ...
      <!-- Approve Tab -->
      <div v-show="activeTab === 'approve'" class="space-y-6">
        <div>
          <h2 class="text-xl font-semibold text-gray-900 mb-2">
            Pending Approval Requests
          </h2>
          <p class="text-sm text-gray-600">
            Review and approve access requests from users
          </p>
        </div>

        <div v-if="pendingApprovals.length > 0" class="space-y-4">
          <div
          ...
          >
            ...
            <div class="grid grid-cols-2 gap-4 text-sm mb-4">
              ...
              <div v-if="request.type === 'privilege_change'">
                <p class="text-gray-600">Current Privilege</p>
                <p class="font-medium text-gray-900">
                  {{ request.currentPrivilege }}
                </p>
              </div>
              <div v-if="request.type === 'privilege_change'">
                <p class="text-gray-600">Requested Privilege</p>
            ...
            <div class="pt-4 border-t flex gap-3 justify-end">
              <button
                @click="openConfirmModal('reject', request.id)"
                class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-red-600 bg-red-50 rounded-md hover:bg-red-100 transition-colors"
              >
                <AlertCircle class="h-4 w-4" />
                Reject
              </button>
              <button
                @click="openConfirmModal('approve', request.id)"
                class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-md hover:bg-green-700 transition-colors"
              >
                <CheckCircle class="h-4 w-4" />
                Approve
              </button>
        ...
        <div
          v-else
          class="rounded-lg border border-dashed bg-gray-50 p-12 text-center"
        >
          ...
          <p class="text-sm text-gray-600">
            All access requests have been reviewed and approved
          </p>
        </div>
      </div>
        ...
        <div class="relative w-full max-w-md rounded-lg bg-white shadow-xl">
          ...
          <div class="px-6 py-6 space-y-4">
            <p class="text-sm text-gray-600">
              {{
                confirmAction === "approve"
                  ? `Are you sure you want to approve this request from ${confirmRequestData?.userName}? This action cannot be undone.`
                  : `Are you sure you want to reject this request from ${confirmRequestData?.userName}? This action cannot be undone.`
              }}
            </p>

            <div
              v-if="confirmRequestData"
              class="rounded-lg bg-gray-50 p-4 space-y-2 text-sm"
            >
              <div>
                <p class="text-gray-600">Request Type</p>
                <p class="font-medium text-gray-900">
                  {{ confirmRequestData.title }}
                </p>
              </div>
              <div>
                <p class="text-gray-600">User</p>
                <p class="font-medium text-gray-900">
                  {{ confirmRequestData.userName }}
                </p>
              </div>
              <div>
                <p class="text-gray-600">System</p>
                <p class="font-medium text-gray-900">
                  {{ confirmRequestData.system }}
                </p>
              </div>
            </div>
          </div>
      ...
      >
        <div class="relative w-full max-w-md rounded-lg bg-white shadow-xl">
          <div class="border-b px-6 py-4 flex items-center justify-between">
            ...
            <button
              @click="showNewAppPasswordModal = false"
          ...
          <div class="px-6 py-4 space-y-4">
            <div>
              ...
              <input
                id="app-name"
                v-model="newAppPasswordForm.name"
                type="text"
            ...
            <div>
              ...
              <select
                id="app-system"
                v-model="newAppPasswordForm.system"
      ...
      >
        ...
        >
          <div
            class="border-b px-6 py-4 sticky top-0 bg-white flex items-center justify-between"
          >
            <h2 class="text-lg font-semibold text-gray-900">
          ...
          <div class="px-6 py-4 space-y-4">
            ...
            <div v-if="newRequestForm.type === 'privilege_change'">
              <label
                for="req-privilege"
                class="block text-sm font-medium text-gray-900 mb-1"
                >Requested Privilege</label
              >
              <select
                id="req-privilege"
                v-model="newRequestForm.requestedPrivilege"
                class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent"
              >
