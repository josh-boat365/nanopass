...
const privileges = ref([
  ...
  },
  {
    id: 2,
    name: "Editor",
    description: "Can create and edit content but cannot change settings.",
  },
...
const updateFilteredPrivileges = () => {
  ...
  filteredPrivileges.value = privileges.value.filter(
    (privilege) =>
...
const updatePrivilege = () => {
  if (formData.value.name.trim() && selectedPrivilege.value) {
    const index = privileges.value.findIndex(
      (p) => p.id === selectedPrivilege.value.id
    );
    if (index > -1) {
...
const deletePrivilege = () => {
  privileges.value = privileges.value.filter(
    (p) => p.id !== selectedPrivilege.value.id
  );
  updateFilteredPrivileges();
  closeDeleteModal();
    ...
    <div class="p-4 sm:p-6">
      ...
      >
        <div>
          ...
          <p class="text-gray-600 text-xs sm:text-sm mt-1">
            Manage user privileges and roles within the application.
          </p>
        </div>
        <button
          @click="openAddModal"
      ...
      <div class="mb-6 flex flex-col sm:flex-row sm:items-center gap-4">
        <div class="flex-1">
          <input
            v-model="searchQuery"
            @input="updateFilteredPrivileges"
            type="text"
        ...
        <div class="text-xs sm:text-sm text-gray-600 whitespace-nowrap">
          {{ filteredPrivileges.length }} result{{
      ...
      <div class="rounded-lg border bg-white shadow-sm overflow-hidden">
              ...
              >
                ...
                >
                  <div class="flex flex-col sm:block">
                    <span class="font-semibold">{{ privilege.name }}</span>
                ...
                <td class="px-4 sm:px-6 py-4 text-right">
                  <div class="flex items-center justify-end gap-1 sm:gap-2">
                    ...
                    <button
                      @click="openDeleteModal(privilege)"
        ...
        <!-- Empty state -->
        <div
          v-if="filteredPrivileges.length === 0"
          ...
          <p class="text-gray-500 text-xs sm:text-sm">
            {{
              searchQuery.trim()
                ...
                : "No privileges found. Create your first one."
            }}
          </p>
        </div>
      </div>
